...
function SettingsPage() {
    ...
    try {
      ...
      Modal.confirm({
        ...
        onOk: async () => {
          ...
          try {
            const migrateResult =
              await window.settingsAPI.migrateNotes(newDirectory);
            ...
            if (migrateResult.movedCount > 0) {
              message.success(
                `已迁移 ${migrateResult.movedCount} 篇日记到新目录`,
              );
            } else {
  ...
  const handleResetDirectory = () => {
    ...
    Modal.confirm({
      title: "恢复默认目录",
      content:
        "日记文件将恢复到默认的应用数据目录下。已有笔记将从当前自定义目录迁移回默认位置。",
      okText: "确认恢复",
      cancelText: "取消",
      onOk: async () => {
        setIsMigratingNotes(true);
        try {
           ...
           */
          const migrateResult = await window.settingsAPI.migrateNotes(
            "__RESET_TO_DEFAULT__",
          );
          if (!migrateResult.success) {
          ...
          if (migrateResult.movedCount > 0) {
            message.success(
              `已迁移 ${migrateResult.movedCount} 篇日记到默认目录`,
            );
          } else {
      ...
      <div className={styles.settingsScrollArea}>
        <div className={styles.settingsContent}>
          ...
          >
            {settingsError ? (
              <Alert
                className={styles.settingsAlert}
              ...
              <Form.Item label="日记文件目录">
                <Space.Compact style={{ width: "100%" }}>
                  <Input
                    readOnly
                    value={
                      storageInfo?.notesPath ??
                      (isLoadingStorageInfo ? "读取中..." : "")
                    }
                  />
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleCopyPath(storageInfo?.notesPath, "日记文件目录")
                    }
                  >
                    复制
                  </Button>
